...
---
## 웹사이트 접속 과정
> `https://spring-community.kro.kr/` 로 접속할 때 발생하는 일련의 과정은 다음과 같습니다. 이 과정은 사용자가 웹 사이트에 접속하는 순간부터 서버가 응답을 반환하기까지의 단계를 포함합니다.
>
> 1. **도메인 이름 해석(DNS Lookup)**: 사용자가 웹 브라우저에 `https://spring-community.kro.kr/` 를 입력하고 접속을 시도하면, 먼저 사용자의 디바이스는 `spring-community.kro.kr`의 IP 주소를 알아내기 위해 DNS 조회를 시작합니다. DNS 서버는 도메인 이름을 해당 서버의 IP 주소로 변환해 줍니다.
> 
> 2. **TCP 연결**: DNS 조회를 통해 IP 주소를 얻은 후, 사용자의 웹 브라우저는 해당 IP 주소로 TCP 연결을 시도합니다. `https://` 스킴을 사용했기 때문에, 기본적으로 TCP 포트 443을 대상으로 연결을 시도합니다.
> 
> 3. **TLS 핸드셰이크**: TCP 연결이 성공하면, 클라이언트와 서버 간에 TLS(Transport Layer Security) 핸드셰이크가 이루어져 암호화된 연결이 수립됩니다. 이 과정에서 서버는 클라이언트에게 SSL/TLS 인증서를 제공합니다. 클라이언트는 이 인증서를 검증하여, 연결이 안전한지 여부를 판단합니다. 인증서가 유효하고 신뢰할 수 있는 기관에 의해 발급된 것이면, 안전한 연결이 확립됩니다.
> 
> 4. **HTTP 요청**: TLS 연결이 성공적으로 수립되면, 사용자의 웹 브라우저는 `https://spring-community.kro.kr/` 에 대한 HTTP GET 요청을 서버로 보냅니다. 이 요청은 암호화되어 전송됩니다.
> 
> 5. **서버 처리**: 요청을 받은 서버는 해당 요청을 처리합니다. Nginx와 같은 웹 서버는 설정에 따라 요청을 특정 애플리케이션 서버로 전달할 수 있습니다. 예를 들어, Nginx 설정에 따라 `https://spring-community.kro.kr/` 로 들어온 요청을 로컬에서 실행 중인 Docker 컨테이너의 8080 포트로 프록시할 수 있습니다.
> 
> 6. **응답 생성 및 전송**: 서버 또는 서버에 연결된 애플리케이션은 요청에 대한 응답을 생성합니다. 이 응답은 HTML 문서, 이미지, 스타일 시트 등 웹 페이지를 구성하는 리소스를 포함할 수 있습니다. 생성된 응답은 다시 클라이언트로 전송됩니다.
> 
> 7. **웹 페이지 렌더링**: 클라이언트의 웹 브라우저는 받은 응답을 해석하여 화면에 웹 페이지를 표시합니다. 필요한 추가 리소스(예: 이미지, CSS, JavaScript 파일 등)가 있다면, 이를 다시 서버에 요청하여 받아옵니다.
> 
> 8. **세션 종료**: 웹 페이지가 완전히 로드되고 모든 리소스가 불러와지면, TLS 연결은 종료될 수 있습니다. 다만, 현대 웹 애플리케이션에서는 AJAX 요청 등을 통해 서버와의 연결을 지속적으로 유지하기도 합니다.